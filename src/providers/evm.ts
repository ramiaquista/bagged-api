 ...
 * (filterWashTrades -> computeCostBasis -> resolveRugs, see TODO(merge)
 * comments below) -> WalletPnl / Position[].
 *
...
export class EvmProvider implements ChainProvider {
  ...
  private async loadPortfolio(address: string, alchemy: AlchemyClient) {
    ...
    // TODO(merge): depends on pnl-engine cost-basis implementation landing
    // from item-2-solana-provider branch. filterWashTrades is currently a
    // stub (src/pnl-engine/washTrade.ts) that returns every trade
    // unfiltered -- excludedCount is always 0 until that lands.
    const washResult = filterWashTrades(rawTrades);

    // TODO(merge): depends on pnl-engine cost-basis implementation landing
    // from item-2-solana-provider branch. resolveRugs is currently a stub
    // (src/pnl-engine/rugResolution.ts) that always returns
    // { resolvedCount: 0, realizedLossUsd: 0 } regardless of input.
    ...
    for (const [tokenAddress, tokenTrades] of byToken) {
      // TODO(merge): depends on pnl-engine cost-basis implementation
      // landing from item-2-solana-provider branch. computeCostBasis is
      // currently a stub (src/pnl-engine/costBasis.ts) that always returns
      // { quantityHeld: 0, costBasisUsd: 0, realizedPnlUsd: 0 } regardless
      // of the trades passed in -- so real EVM wallets show zeroed-out PnL
      // and no open positions until that lands, even though the Trade[]
      // fed into it here (including preGraduation tagging) is real.
